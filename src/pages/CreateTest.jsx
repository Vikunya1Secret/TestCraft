...
// import mapCardTypeToQuestionType from ""
import mapCardTypeToQuestionType, { answerGenerator } from "../lib/util";
...
const CreateTest = () => {
  ...
  const handleSaveTest = () => {
    ...
    createTest({
      ...
      passingScore: testData.questions.length,
    }).then((d) => {
        ...
        <div className="flex gap-6">
          ...
          <div className="w-2/3 bg-[#f7faff] rounded-3xl p-8">
            ...
            <button
              ...
              className="w-full bg-[#F68D88] cursor-pointer h-[52px] text-white rounded-full text-[18px] py-3 font-bold"
            >
              Сохранить тест
            </button>
          </div>
          ...
          <div className="w-1/3 px-[44px] rounded-2xl bg-[#f7faff]">
                ...
                <div className="relative inline-block w-10 h-5 rounded-full">
                  ...
                  <label
                    ...
                    className={`absolute cursor-pointer top-0 left-0 right-0 bottom-0 rounded-full transition-all duration-300 ${
                      manualCheck ? "bg-[#3d568f]" : "bg-gray-300"
                    }`}
                  >
                    <span
                      className={`absolute h-4 w-4 left-0.5 bottom-0.5 bg-white rounded-full transition-all duration-300 ${
                        manualCheck ? "translate-x-5" : "translate-x-0"
                      }`}
                    ></span>
                  </label>
                </div>
            ...
            {/* Add Question Button */}
            <div onClick={toggleCardSelection} className="bg-white cursor-pointer rounded-3xl relative p-4">
              <div className="flex items-center w-[254px] gap-2 mb-1">
                <button
                  className="w-8  h-8 bg-[#95b1ee] rounded-full flex items-center justify-center text-white"
                  
                >
                  <Plus className="w-4 h-4" />
                </button>
                <span className="text-[#4a4f55] text-[17px]">
              ...
              {showCardSelection && (
                <div className="absolute w-[267px] flex flex-col gap-y-5 shadow-lg rounded-2xl bg-white p-6 z-10">
                  <div
